...
public:
	...
	void insereFimLista(T * elemento); //modificado
	void inserirInicioLista(T * elemento); //modificado
	...
	void esvaziaLista();
	bool contemNaLista(T * elemento, No<T> * param_lista);
...
void Lista<T>::inserirInicioLista(T * elemento)
{
	...
	if (!listaVazia()) {
		no->proximo = lista;//Atualiza próximo para o início da lista.
		lista = no; //Atualiza lista para nova alocação dinâmica.
		tam++; //Adiciona + 1 elemento para o tamanho da lista.
	}
	else {
		lista = no; //Se lista está vazia.
		tam++;
	}
}
template <class T>
...
template<class T>
bool Lista<T>::contemNaLista(T * elemento, No<T> * param_lista)
{
	//recebe o nó
