...
public:
	...
	void esvaziaLista();
	bool contemNaLista(const T* elemento); //true = contem na lista; false = não contêm na lista
	bool comparaLista(Lista<T> * lista_comparar); //true = listas são iguais; false = listas são diferentes.
...
inline bool Lista<T>::comparaLista(Lista<T>* lista_comparar)
{
	...
	if (lista_comparar != nullptr) {
		if (this->obtemTamanhoLista() != lista_comparar->obtemTamanhoLista()) {
			...
			for (int i = 0; i < lista_comparar->obtemTamanhoLista(); i++) {
				if (obterElementoEm(i) != lista_comparar->obterElementoEm(i)) {
					return false;
				}
			}
			return true;
